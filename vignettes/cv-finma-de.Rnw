...
% daniehei
\usepackage{lipsum}
\usepackage[a4paper, top=2.5cm, bottom=3cm, right=2cm]{geometry}
...
\begin{cvlist}{\spacedlowsmallcaps{Stellenprofil}}\label{stellenprofil}
\item
\end{cvlist}
\vspace{-2em}
\begin{tabular}{p{6cm}p{9cm}}
\textbf{Anforderung} & \textbf{Meine Qualifikation}\\
Aus-/Weiterbiludng in Informatik oder Wirtschaftsinformatik & Doktorat mit starkem quantitativen, daten- und technikorientierten Fokus (ETH Zürich); MSc in Ökonomie mit quantitativer Vertiefung; 5 Jahre Selbststudium Programmieren\\
Beratung von Stakeholdern, Projektkoordination & Mehrjährige Erfahrung mit Stakeholdern aus Forschung und Praxis, Projektleitung\\
Datenerhebung, Gestaltung von Erhebungen & Umfangreiche Erfahrung mit komplexer Umfragekonzeption und -durchführung, sowie anschliessende Datenanalyse und Prozessautomatisierung\\
Analytisches Denkvermögen und Problemlösungskompetenz & Mehrjährige Forschungstätigkeit, \R-Paket-Entwicklung und Beteiligung an verschiedenen Open-Source Projekten\\
Programmierkenntnisse und schnelle Auffassungsgabe & \R, Python, Bash, HTML, CSS, JavaScript, SQL, Git, \LaTeX -- Fähigkeit und Wille, neue Konzepte/Sprachen schnell anzueignen\\
Fundierte Kenntnisse in XML / XSD / T-SQL / XPath & Kenntnis diverser Markup-Sprachen, Serialization und Dezerialization, Object Relational Model, etc. Motivation allfällige Lücken schnell zu schliessen\\
Datenbank- und Servererfahrung & Geübt in SQL, Unix-shell, ssh, etc.\\
Gute Sprachkenntnisse & Deutsch (Muttersprache); Englisch (C1); Schwedisch (B2); Französisch (B1)\\
\end{tabular}
...
\item \textbf{Software:} \proglang{R}, Python, \LaTeX, git (und GitHub), bash, Docker, sql, Webtechnologien wie HTML, CSS (Tailwind), JavaScript, diverse Markup-Sprachen, etwas C/C++
