...
Aus-/Weiterbiludng in Informatik oder Wirtschaftsinformatik & Doktorat mit starkem quantitativen, daten- und technikorientierten Fokus (ETH Zürich); MSc in Ökonomie mit quantitativer Vertiefung\\
Beratung von Stakeholdern, Projektkoordination & Mehrjährige Erfahrung mit Stakeholdern aus Forschung und Praxis (z.B. SBB, AMAG, Tiefbauamt), Projektleitung\\
Datenerhebung, Gestaltung von Erhebungen & Umfangreiche Erfahrung mit Stated-Preference-Umfragen und Datenanalyse\\
Analytisches Denkvermögen und Problemlösungskompetenz & Stark belegt durch deine Forschung, deine R-Pakete und Open-Source-Projekte\\
Programmierkenntnisse und schnelle Auffassungsgabe & R, Python, Bash, SQL, Git, LaTeX – sehr breit aufgestellt; du bist zudem Entwickler eigener Tools\\
Datenbank- und Servererfahrung & PostgreSQL, Unix-shell etc. -- vorhanden\\
Gute Sprachkenntnisse (Deutsch C2, Englisch B2) & Deutsch: Muttersprache; Englisch: C1\\
\end{tabular}
