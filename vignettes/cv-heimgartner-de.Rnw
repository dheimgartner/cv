    ...
    \begin{cv}{\spacedallcaps{Curriculum Vit\ae}}
        ...
        \vspace{-3em}
            \begin{itemize}[nosep]
              ...
              \item Contributor bei verschiedenen Projekten: z.B. \pkg{mixl} (wie auch Kirill Müller), \pkg{XRPython} (zusammen mit John Chambers), und auch ausserhalb \R~(z.B. \pkg{yolo.nvim} \url{https://github.com/dheimgartner/yolo.nvim})
              \item Teilnehmer an der useR Konferenz in Salzburg
              \end{itemize}
              \item Fundierte Programmierkenntnisse (über \R~hinaus)
              \begin{itemize}[nosep]
              \item Interesse an Web-Technologien (HTMLwidget für Shiny -- SurveyJS Wrapper \url{https://github.com/dheimgartner/surveytools}; Grundkenntnisse in \proglang{JavaScript} und Web-Frameworks wie \proglang{django} oder \proglang{react})
              \item Erfahrung in Server und Datenbank Management (geübt in unix-shell/bash \url{https://github.com/dheimgartner/MOBISarchive/blob/main/main.sh}, ssh, etc.), sowie Docker (z.B. \url{https://github.com/dheimgartner/shiny-deploy}).
              \end{itemize}
              \item Interesse, meine statistischen Modellierungskenntnisse zu vertiefen und weitere Beiträge zu leisten (z.B. Implementierung eines Copula modeling frameworks in \R)
              \item Sinn für (Prozess-) Ästhetik und Motivation, Analyseergebnisse in ansprechender und eleganter Form zu präsentieren
              \item Anliegen, nützliche Erfahrungen und Tools weiter zu geben (Initiant und Dozent des \emph{let's use xxx} Data Science Workshops in unserer Forschungsgruppe \url{https://github.com/orgs/lets-use-xxx/repositories})
              \item Ausgesprochen lernbegeisterte und selbstmotivierte Person
        ...
        \begin{cvlist}{\spacedlowsmallcaps{Ausbildung}}\label{education}
            \item \textbf{Doktorat}, Institut für Verkehrsplanung und Transportsysteme, IVT, ETH Zürich
            \marginpar{2021--heute}
            \begin{itemize}
                \item \emph{Measuring and Modeling the Impact of Telework on Transport Demand -- Data, Tools and Analysis}: Datenerhebung, Stated-Preference Umfragen, Wahlmodellierung, Kausale Inferenz (Schätzung des Treatment effects auf Grundlage von Querschnittsdaten), (Mit-) Entwicklung verschiedener \proglang{R}-Pakete (\pkg{OPSR}, \pkg{mixl}, ...)
            ...
            \end{itemize}
            \item \textbf{Militärdienst als Gebirgsspezialist}, Kompetenzzentrum Gebirgsdienst der Armee, Adermatt
            \marginpar{2012--2013}
            \begin{itemize}
        ...
        \begin{cvlist}{\spacedlowsmallcaps{Berufserfahrung}}\label{work}
            ...
            \begin{itemize}
                ...
                \item Datenaufbereitung und Mithilfe bei ökonomischen Analysen
            \end{itemize}
        ...
        \begin{cvlist}{\spacedlowsmallcaps{Fähigkeiten}}\label{skill}
            \item \textbf{Software:} Hervorragende \proglang{R} Kenntnisse, Python, \LaTeX, git, bash, etwas C (primär durch Lektüre der Bibel \emph{The C Programming Language}) und JavaScript
