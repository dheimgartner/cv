...
\vspace{-3em}
\begin{itemize}[nosep]
\item Experte in kausaler Modellierung (Statistik/Ökonometrie)
...
\item Fundierte Programmierkenntnisse (über \R~hinaus) und schnelle Auffassungsgabe für neue Konzepte, Frameworks und Programmiersprachen
\item Erfahrung in Server und Datenbank Management (geübt in unix-shell/bash, ssh, PostgreSQL, etc.)
...
\item Anliegen, nützliche Erfahrungen und Tools weiter zu geben: Initiant und Leiter Data Science Workshops in unserer Forschungsgruppe \url{https://github.com/orgs/lets-use-xxx/repositories}
\item Erfahrung in Projektleitung (Koordination verschiedener Stakeholder aus Akademie und Praxis)
