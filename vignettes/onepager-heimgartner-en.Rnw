...
\begin{itemize}
\item \emph{Measuring and Modeling the Impact of Telework on Transport Demand -- Data, Tools and Analysis}: Data collection, stated-preference surveys, choice modeling, causal inference (estimation of treatment effect using cross-sectional data), (co-) development of several \proglang{R} packages (\pkg{OPSR}, \pkg{mixl}, ...)
\end{itemize}
...
\marginpar{2012--2013}
\begin{itemize}
\item Winter basic training (training in mountain sports for summer \& winter); completed all refresher courses; various ski, alpine, and climbing tours; implementation and leadership of (international) training and education
\end{itemize}
